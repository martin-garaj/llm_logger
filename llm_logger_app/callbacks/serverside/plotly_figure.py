...
def register_render_test_figure(app):
    ...
    def render_test_figure(trigger):
        ...
        # aspect ratio 
        aspect_ratio = f"{parser.aspect_ratio:.3f}" #json.dumps(dict(aspect_ratio=parser.aspect_ratio))
        
        
        return figure, chapter_locations, chapter_buttons, aspect_ratio

def register_update_positions_json(app):
    @app.callback(
        Output('chapter-position-json', 'children'),
